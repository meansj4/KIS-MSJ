...
- `decimal_rate`: UI에서는 percent로 보여주고 저장 시 100으로 나눈 소수로 저장합니다.
  - 예: `normal_reentry_drop_rate` UI `5.0%` -> config `0.05`
...
- `strategy.normal_reentry_drop_rate`: 전량 PROFIT_TAKE 후 일반 재진입 기본 기준입니다. 기본 5% 하락에서 시작해 30 calendar day 동안 하루 약 0.1667%p씩 완화되어 0%에서 cap 됩니다. 기준가격은 전량 매도 사이클의 SELL 체결 VWAP과 median 중 낮은 값인 `normal_exit_anchor_price`입니다.
- `strategy.trailing_activation_gain` / `strategy.trailing_reentry_drop_rate`: 전량 매도 후 더 올라간 종목을 고점 대비 조정 시 다시 보는 trailing 재진입 기준입니다. 기본 10% 하락에서 시작해 30 calendar day 동안 하루 약 0.3333%p씩 완화되어 0%에서 cap 됩니다. 활성화 기준가격은 SELL 체결 VWAP과 median 중 높은 값인 `trailing_exit_anchor_price`입니다.
...
## New Season KIS balance snapshot generation

The New Season tab now has a `Generate KIS balance snapshot` button. The button performs a read-only KIS balance inquiry, writes `exports/kis_balance_snapshot_YYYYMMDD_HHMMSS.json`, validates the file immediately, and fills the snapshot path in the UI. It does not create, revise, cancel, or submit orders.

Operator flow:

1. Open the New Season tab and click `Generate KIS balance snapshot`.
2. Check the creation result and validator result.
3. Confirm the snapshot contains fresh `generated_at` and actual `sellable_quantity` values.
4. Use the filled snapshot path to create the liquidation plan.
