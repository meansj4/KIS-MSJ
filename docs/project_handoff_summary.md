...
결과는 `153 passed`였고, pytest cache warning 1개는 기능 실패가 아니다.
...
- KIS balance snapshot 설명은 “현재는 JSON 파일 입력 검증 구조이며 자동 생성 기능은 없음, 운영자가 별도 JSON을 준비한다. 실제 request 생성 단계에서는 `generated_at`과 `sellable_quantity` 필수”로 통일한다.
