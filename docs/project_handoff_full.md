...
기준 테스트 결과: `153 passed` (`.\.venv\Scripts\python.exe -m pytest -q --basetemp .pytest_tmp_final_logic_check`)  
...
| 최신 테스트 결과 | `153 passed`, pytest cache warning 1개는 기능 실패 아님 |
...
| 가격 구간 | 1 LOT 금액 | 종목당 최대금액 | max_lots | enabled | note |
| --- | ---: | ---: | --- | --- | --- |
| 0~300 | 0 | 0 |  | false | 초저가주는 자동매수 제외 또는 paper 전용 |
...
| 300,001~1,000,000 | 1,000,000 | 3,000,000 | 3 | true | 고가주는 3 LOT까지만 |
| 1,000,001~3,000,000 | 0 | 0 |  | false | 초고가주는 자동매수 제외, manual only |
...
최신 확인 기준 전체 테스트는 다음 명령에서 `153 passed`였다. pytest cache warning 1개는 basetemp/cache write 관련이며 기능 실패가 아니다.
...
| 기대 결과 | 최신 기준 `153 passed`. warning 1개는 pytest cache 관련이면 기능 실패 아님 |
...
| 가격 구간 | 1 LOT | 종목당 최대 | enabled | 위험/영향 |
| --- | ---: | ---: | --- | --- |
| 0~300 | 0 | 0 | false | 초저가 자동매수 제외 |
| 301~1,000 | 3,000 | 30,000 | true | 소액 LOT |
| 1,001~10,000 | 10,000 | 100,000 | true | 중저가 |
| 10,001~30,000 | 30,000 | 300,000 | true | 기존 3만원 LOT 기준 |
| 30,001~100,000 | 100,000 | 1,000,000 | true | 고가 진입액 증가 |
| 100,001~300,000 | 300,000 | 3,000,000 | true | 큰 LOT, 총액 제한 주의 |
| 300,001~1,000,000 | 1,000,000 | 3,000,000 | true, max_lots=3 | 매우 큰 LOT |
| 1,000,001 이상 | 0 | 0 | false | 자동매수 제외/manual only |
...
| 테스트 최신성 | `153 passed`, pytest cache warning 1개는 기능 실패 아님. 실행 시점에는 다시 확인 필요. |
