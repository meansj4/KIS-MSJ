...
기준 테스트 결과: `143 passed` (`.\.venv\Scripts\python.exe -m pytest -q --basetemp .pytest_tmp_final_logic_check`)  
...
| 최신 테스트 결과 | `143 passed`, pytest cache warning 1개는 기능 실패 아님 |
...
최신 확인 기준 전체 테스트는 다음 명령에서 `143 passed`였다. pytest cache warning 1개는 basetemp/cache write 관련이며 기능 실패가 아니다.
...
| 기대 결과 | 최신 기준 `143 passed`. warning 1개는 pytest cache 관련이면 기능 실패 아님 |
...
| 테스트 최신성 | `143 passed`, pytest cache warning 1개는 기능 실패 아님. 실행 시점에는 다시 확인 필요. |
