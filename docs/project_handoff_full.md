...
> UI update: the local UI now has a read-only `운용 현황 Portfolio/Risk` tab backed by `GET /api/portfolio-dashboard`. It summarizes total/current LOT exposure, realized/unrealized PnL, daily fill performance, limit usage progress bars, top symbol exposure, and risk/review/sync counts. It does not call KIS order APIs, create orders, mutate DB trading rows, or reset DB. Pre/After/Night market-session expansion is on hold as a separate branch/review item; current main operation remains regular-session oriented. See [portfolio_dashboard.md](portfolio_dashboard.md).
...
> 2026-05-29 final loop optimization policy: automatic evaluation changed from 5 stability samples for every enabled symbol to one scan quote for every enabled symbol, then 5 stability samples only for BUY/SELL candidates, then one final quote before request construction. Operators should accept that non-candidates skip stability sampling, candidates may be blocked more conservatively, and final quote can change quantity/limit/new unlocked LOT bucket. Rollback range: `0e8405d..42c37fe`. Final quote is traceable through `decisions.current_price`, linked `price_snapshots.current_price`, and submitted `orders` rows. Current `raw_json` records `sample_count` only; it does not store individual stability samples or a nested `final_quote` field.
