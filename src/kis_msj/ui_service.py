...
CONFIG_METADATA: tuple[dict[str, Any], ...] = (
    ...
    {"section": "Strategy", "key": "strategy.normal_reentry_drop_rate", "label_ko": "NORMAL_REENTRY 기본 하락률", "description_ko": "normal_exit_anchor_price 대비 최초 -5% 기준이며 WAIT_REENTRY 경과일 decay 후 유효 기준으로 재진입 후보가 됩니다.", "type": "number", "unit": "%", "display_format": "decimal_percent", "config_format": "decimal_rate", "warning_level": "normal", "requires_restart": True},
    ...
    {"section": "Strategy", "key": "strategy.trailing_reentry_drop_rate", "label_ko": "TRAILING 기본 고점 대비 하락률", "description_ko": "post_exit_high_price 대비 최초 -10% 기준이며 WAIT_REENTRY 경과일 decay 후 유효 기준으로 trailing 재진입 후보가 됩니다.", "type": "number", "unit": "%", "display_format": "decimal_percent", "config_format": "decimal_rate", "warning_level": "normal", "requires_restart": True},
    ...
    {"section": "Strategy", "key": "strategy.reentry_decay_enabled", "label_ko": "재진입 decay 활성화", "description_ko": "true이면 WAIT_REENTRY calendar day 경과에 따라 normal/trailing 재진입 기준을 0%까지 완화합니다.", "type": "boolean", "unit": "bool", "display_format": "boolean", "config_format": "boolean", "warning_level": "normal", "requires_restart": True},
    {"section": "Strategy", "key": "strategy.normal_reentry_decay_days", "label_ko": "NORMAL decay 기간", "description_ko": "NORMAL_REENTRY 기준을 0%까지 완화하는 calendar day 기간입니다.", "type": "number", "unit": "일", "display_format": "integer", "config_format": "integer", "warning_level": "normal", "requires_restart": True},
    {"section": "Strategy", "key": "strategy.normal_reentry_daily_decay_rate", "label_ko": "NORMAL 일 decay", "description_ko": "NORMAL_REENTRY 유효 기준에 매 calendar day 더하는 rate입니다. 기본값 0.0016666667은 하루 약 0.1667%p 완화입니다.", "type": "number", "unit": "%p/day", "display_format": "decimal_percent", "config_format": "decimal_rate", "warning_level": "normal", "requires_restart": True},
    {"section": "Strategy", "key": "strategy.trailing_reentry_decay_days", "label_ko": "TRAILING decay 기간", "description_ko": "TRAILING_REENTRY 기준을 0%까지 완화하는 calendar day 기간입니다.", "type": "number", "unit": "일", "display_format": "integer", "config_format": "integer", "warning_level": "normal", "requires_restart": True},
    {"section": "Strategy", "key": "strategy.trailing_reentry_daily_decay_rate", "label_ko": "TRAILING 일 decay", "description_ko": "TRAILING_REENTRY 유효 기준에 매 calendar day 더하는 rate입니다. 기본값 0.0033333333은 하루 약 0.3333%p 완화입니다.", "type": "number", "unit": "%p/day", "display_format": "decimal_percent", "config_format": "decimal_rate", "warning_level": "normal", "requires_restart": True},
    {"section": "Strategy", "key": "strategy.reentry_decay_cap_rate", "label_ko": "재진입 decay 상한", "description_ko": "유효 재진입 기준의 상한입니다. 기본 0%로 양수 threshold가 되지 않게 막습니다.", "type": "number", "unit": "%", "display_format": "decimal_percent", "config_format": "decimal_rate", "warning_level": "normal", "requires_restart": True},
    {"section": "Strategy", "key": "strategy.force_reentry_after_timeout_enabled", "label_ko": "30일 timeout 강제 재진입", "description_ko": "true이면 WAIT_REENTRY가 timeout일을 넘고 조건 재진입이 없을 때 새 initial cycle BUY 후보를 만듭니다.", "type": "boolean", "unit": "bool", "display_format": "boolean", "config_format": "boolean", "warning_level": "warning", "requires_restart": True},
    {"section": "Strategy", "key": "strategy.force_reentry_timeout_days", "label_ko": "강제 재진입 timeout", "description_ko": "WAIT_REENTRY 시작일(exit_time)부터 이 calendar day 이상 지나면 force reentry 후보가 됩니다.", "type": "number", "unit": "일", "display_format": "integer", "config_format": "integer", "warning_level": "warning", "requires_restart": True},
    {"section": "Strategy", "key": "strategy.force_reentry_starts_new_cycle", "label_ko": "강제 재진입 새 cycle", "description_ko": "true이면 force reentry BUY 체결은 기존 anchor/lot sizing을 carry-over하지 않고 현재가 기준 새 cycle로 처리합니다.", "type": "boolean", "unit": "bool", "display_format": "boolean", "config_format": "boolean", "warning_level": "warning", "requires_restart": True},
...
DETAILED_CONFIG_DESCRIPTIONS: dict[str, str] = {
    ...
    "strategy.normal_reentry_drop_rate": "PROFIT_TAKE 전량 매도 후 WAIT_REENTRY 상태에서만 쓰는 일반 재진입 기본 하락률입니다. 기준 가격인 normal_exit_anchor_price는 해당 보유 사이클의 SELL 체결가들로 계산한 cycle_sell_vwap_price와 cycle_sell_median_price 중 낮은 값입니다. 기본 -5%에서 calendar day decay를 적용해 최대 0%까지 완화됩니다.",
    ...
    "strategy.trailing_reentry_drop_rate": "TRAILING_REENTRY의 고점 대비 기본 조정폭입니다. 전량 매도 후 새 고점(post_exit_high_price)이 형성되고 trailing_activation_gain 조건을 만족한 뒤, 현재가가 그 고점에서 유효 기준 이상 내려오면 TRAILING_REENTRY 후보가 됩니다. 기본 -10%에서 calendar day decay를 적용해 최대 0%까지 완화됩니다.",
...
def _lot_sizing_config_metadata() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "description_ko": "가격대별 band에 max_lots가 따로 없을 때 적용하는 기본 최대 OPEN LOT 수입니다. 기본값은 12개입니다. 이 수에 도달하면 BUY는 max_lots_per_symbol_reached로 차단되고 SELL은 계속 허용됩니다.",
            "type": "number",
            "unit": "개",
