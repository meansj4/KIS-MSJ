...
CONFIG_METADATA: tuple[dict[str, Any], ...] = (
    ...
    {"section": "Strategy", "key": "strategy.reference_exclusion_enabled", "label_ko": "추가매수 기준가 손실 LOT 제외", "description_ko": "true이면 추가매수 기준가 계산에서 큰 손실 OPEN LOT을 제외합니다.", "type": "boolean", "unit": "bool", "display_format": "boolean", "config_format": "boolean", "warning_level": "warning", "requires_restart": True},
    {"section": "Strategy", "key": "strategy.reference_exclusion_loss_rate", "label_ko": "추가매수 기준가 제외 손실률", "description_ko": "OPEN LOT의 현재 손익률이 이 값 이하이면 VWAP/median/reference 계산에서 제외합니다.", "type": "number", "unit": "%", "display_format": "decimal_percent", "config_format": "decimal_rate", "warning_level": "warning", "requires_restart": True},
...
DETAILED_CONFIG_DESCRIPTIONS: dict[str, str] = {
    ...
    "strategy.reference_exclusion_enabled": "추가매수 기준가(reference_buy_price)를 계산할 때 큰 손실 OPEN LOT을 제외할지 정합니다. 기본 true이면 손익률이 reference_exclusion_loss_rate 이하인 LOT은 VWAP/median/reference 계산에서 빠지고, 기존 cycle lock 자체는 유지됩니다.",
    "strategy.reference_exclusion_loss_rate": "추가매수 기준가 계산에서 제외할 LOT 손실률입니다. 기본 -15%입니다. 예: -0.15이면 현재가 기준 손익률이 -15.0% 이하인 OPEN LOT을 제외합니다. 모든 OPEN LOT이 제외되면 reference_buy_price는 현재가로 fallback되어 즉시 추가매수를 더 보수적으로 막습니다.",
