...
def test_config_snapshot_and_decision_are_persisted(tmp_path):
    ...
    with sqlite3.connect(config.storage_path) as connection:
        snapshot = connection.execute("SELECT config_hash, source FROM config_snapshots").fetchone()
        decision = connection.execute("SELECT config_hash, code, payload_json FROM decisions").fetchone()

    assert snapshot[0] == expected_hash
    assert snapshot[1] == "bot_init"
    assert decision[0] == expected_hash
    assert decision[1] == "005930"
    payload = json.loads(decision[2])
...
def test_orders_fills_and_manual_requests_keep_config_hash(tmp_path):
    ...
    store.set_active_config("HASH-1", "VERSION-1")
    ...
    with sqlite3.connect(store.path) as connection:
        order_hash = connection.execute("SELECT config_hash, config_version FROM orders").fetchone()
        fill_hash = connection.execute("SELECT config_hash, config_version FROM fills").fetchone()
        manual_hash = connection.execute("SELECT config_hash, config_version FROM manual_order_requests").fetchone()

    assert tuple(order_hash) == ("HASH-1", "VERSION-1")
    assert tuple(fill_hash) == ("HASH-1", "VERSION-1")
    assert tuple(manual_hash) == ("HASH-1", "VERSION-1")
